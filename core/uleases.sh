...
# LOCAL USER detection (fallback when LOCAL_USER isn't set in uhotspot.conf)
detect_local_user() {
...
class "blockdhcp" {
     match pick-first-value (option dhcp-client-identifier, hardware);
        ...
        # Deactivated managed MACs (commented "#a;mac;..." lines in mac-*.txt)
        # join blockdhcp.txt entries in the same "blockdhcp" class: pydhcpd
        # denies them a lease outright, exactly like a blocked MAC.
        {
            cut -d ';' -f 2 "$ACL_BLOCK_FILE" 2>/dev/null
            grep -h '^#a;' "$ACL_MAC_PATH"/mac-* 2>/dev/null | cut -d ';' -f 2 || true
